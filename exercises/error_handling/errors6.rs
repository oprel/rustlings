...
fn parse_pos_nonzero(s: &str) -> Result<PositiveNonzeroInteger, ParsePosNonzeroError> {
    ...
    if let Err(parse_error) = parsed{
        return Err(ParsePosNonzeroError::from_parseint(parse_error))
    }
    let x = parsed.unwrap();
    let created: Result<PositiveNonzeroInteger, CreationError> = PositiveNonzeroInteger::new(x);
    if let Err(creation_error) = created{
        return Err(ParsePosNonzeroError::from_creation(creation_error));
    }
